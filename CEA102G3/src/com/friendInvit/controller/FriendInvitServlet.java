			...
			if (memNoStr != null && !memNoStr.isEmpty()) {
				...
				for (FriendInvitVO friendInvitVO : listFriendInvit) {
					...
					try {
						...
						obj.put("imgTag", "<img src='" + req.getContextPath() + "/GetPicture?id=" + adderVO.getMemberNo()
								+ "' style='width: 50px; height: 50px;'>");
						obj.put("comfirmToFriend", "<button class=\"btn btn-outline-primary\" id=\"comfirmToFriendBtn\" name=\"comfirmToFriendBtn\">確認</button>");
						array.put(obj);
					} catch (JSONException e) {
						e.printStackTrace();
					}
