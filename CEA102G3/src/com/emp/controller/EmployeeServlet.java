	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllNews.jsp的請求
			...
			try {
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/employee/listAllEmp.jsp");
				failureView.forward(req, res);
			}
		}
...
//				employeeVO.setEmplo_No(emplo_No);
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("employeeVO", employeeVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/employee/addEmp.jsp");
					failureView.forward(req, res);
					return;
				}
		...
		if ("delete".equals(action)) { // 來自listAllNews.jsp
			...
			try {
				/***************************1.接收請求參數***************************************/
				String[] emplo_Nos = req.getParameterValues("emplo_No");
				EmployeeService empSvc = new EmployeeService();
				for(int i = 0; i < emplo_Nos.length;i++) {
					empSvc.deleteEmployee(new Integer(emplo_Nos[i]));
				}
				//				Integer emplo_No = new Integer(req.getParameter("emplo_No"));
